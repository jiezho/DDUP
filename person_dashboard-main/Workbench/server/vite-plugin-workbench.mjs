...
export function workbenchApiPlugin({
  ...
  let localDomainApi = null;
  let localDomainBootstrapToken = null;
  ...
  async function ensureLocalDomainApi() {
    if (!localDomainApi) {
      localDomainBootstrapToken = randomBytes(32).toString("base64url");
      const dataDirectory = path.resolve(
        process.env.WORKBENCH_DATA_DIR || path.join(workbenchRoot, ".workbench-data"),
      );
      mkdirSync(dataDirectory, { recursive: true });
      localDomainApi = createWorkbenchApp({
    ...
    return localDomainApi;
  }

  async function closeLocalDomainApi() {
    const app = localDomainApi;
    localDomainApi = null;
    localDomainBootstrapToken = null;
    if (app) await app.close();
  }

  function isExactSameOriginRequest(req) {
    if (typeof req.headers.origin !== "string" || typeof req.headers.host !== "string") return false;
    try {
      const origin = new URL(req.headers.origin);
      return origin.protocol === "http:" && origin.host.toLowerCase() === req.headers.host.toLowerCase();
    } catch {
      return false;
    }
  }
  ...
  return {
    ...
    async closeBundle() {
      ...
      await closeLocalDomainApi();
    },
    configureServer(server) {
      vaultSync.attachWatcher(server.watcher);
      server.httpServer?.once("close", () => {
        void vaultSync.close();
        void closeLocalDomainApi();
      });
      server.middlewares.use(async (req, res, next) => {
        const url = new URL(req.url || "/", "http://127.0.0.1");
        if (!url.pathname.startsWith("/api/")) return next();

        if (url.pathname === "/api/health" || url.pathname.startsWith("/api/v1/")) {
          try {
            const app = await ensureLocalDomainApi();
            if (
              req.method === "POST" &&
              url.pathname === "/api/v1/session/bootstrap" &&
              isExactSameOriginRequest(req)
            ) {
              req.headers["x-workbench-bootstrap"] = localDomainBootstrapToken;
            }
            app.routing(req, res);
          } catch (error) {
            ...
            return json(res, 503, {
              ...
              status: "error",
              data: null,
              errors: [{ code: "SERVICE_DEGRADED", message: "项目服务暂时不可用。", field: null, retryable: true }],
              meta: { api_version: "v1" },
            });
          }
          return;
