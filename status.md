...
- 验证：2026-09-15 前端按需加载与构建包体硬门提交 `e5477e0` 已取得 GitHub Actions 7/7 远端证据（流水线 `34988366225`）；入口 258.81 kB、最大分块 464.77 kB，原 1.55 MB 主包告警已消除。本地 275/275、隐私扫描与 Chromium/Firefox/WebKit 端到端各 1/1 通过。生产锁依赖机器清单可复现生成 349 项，其中 50 项需人工许可证/平台元数据复核。
...
- 代码管理：当前远端分支为 `codex/p0-p1-complete`，正式远端目标为 `https://github.com/jiezho/DDUP.git`；WP0–WP5 基线 `c0159cb`、G7/S8 提交 `340c1b9`、Reader 稳定化提交 `0bc362c` 与包体门提交 `e5477e0` 均已推送，最新已完成远端流水线为 `https://github.com/jiezho/DDUP/actions/runs/34988366225`，7/7 通过。`experiments/hermes-poc/` 继续保持未跟踪和排除。
