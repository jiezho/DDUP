...
- 验证：2026-09-14 当前提交 `c0159cb` 的 GitHub Actions Windows/Linux、双平台 SQLite 与 Chromium/Firefox/WebKit 7/7 通过。本轮 S8 修改在 Node 24.19 下完成独立生产构建、273/273 测试与隐私扫描；本地轻量性能预算首轮通过；恢复已增加先完整校验、隔离暂存、SQLite quick check 和原子落盘，专项 3/3 通过。构建仍有 1.55 MB JS 主包的非阻塞提示。
...
- 代码管理：当前远端分支为 `codex/p0-p1-complete`，正式远端目标为 `https://github.com/jiezho/DDUP.git`；WP0–WP5 提交 `c0159cb` 已推送，对应远端流水线为 `https://github.com/jiezho/DDUP/actions/runs/34800589740`。`experiments/hermes-poc/` 继续保持未跟踪和排除。
