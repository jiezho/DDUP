...
- 验证：2026-09-17 新增构建物只读 CLI 校验、构建后自校验及真实构建物复制/篡改测试；本地 284/284、独立生产构建和隐私扫描通过。提交 `d5663ab` 的 GitHub Actions 7/7 通过，流水线 `35173260742`。此前历史 v13 存储模块拒绝 v15 数据库、独立进程正式 CLI 恢复专项 9/9。入口 258.81 kB、最大分块 464.77 kB；构建固定 131 个文件、18,258,965 字节与聚合 SHA-256。生产锁依赖共 349 项，其中 50 项仍需人工许可证/平台元数据复核。
...
- 代码管理：当前远端分支为 `codex/p0-p1-complete`，正式远端目标为 `https://github.com/jiezho/DDUP.git`；历史代码演练 `086c6f6`、其审计记录 `98c672a` 与复制后只读验收 `d5663ab` 均已推送，最新已完成远端流水线为 `https://github.com/jiezho/DDUP/actions/runs/35173260742`，7/7 通过。`experiments/hermes-poc/` 继续保持未跟踪和排除。
