...
- 验证：2026-09-16 构建物哈希与恢复故障门提交 `6a1693e` 已取得 GitHub Actions 7/7 远端证据，流水线 `34994540420`；Windows/Linux、双平台 SQLite 与 Chromium/Firefox/WebKit 全部通过。入口 258.81 kB、最大分块 464.77 kB；本地 281/281、生产构建和隐私扫描通过；恢复容量/中断/竞争/迁移/回退保护专项连续 10 轮共 70/70 通过；构建自动固定 131 个文件、18,258,965 字节与聚合 SHA-256。生产锁依赖共 349 项，其中 50 项仍需人工许可证/平台元数据复核。
...
- 代码管理：当前远端分支为 `codex/p0-p1-complete`，正式远端目标为 `https://github.com/jiezho/DDUP.git`；WP0–WP5 基线 `c0159cb`、G7/S8 提交 `340c1b9`、Reader 稳定化 `0bc362c`、包体门 `e5477e0`、依赖清单 `330392d` 与恢复故障门 `6a1693e` 均已推送，最新已完成远端流水线为 `https://github.com/jiezho/DDUP/actions/runs/34994540420`，7/7 通过。`experiments/hermes-poc/` 继续保持未跟踪和排除。
