...
- 当前阶段：P0/P1 已完成并取得 GitHub Actions 7/7 远端证据；WP0–WP5 首版开发与本地发布门已完成，本轮将形成 Git 提交，远端 CI 需获推送授权后复跑。
...
- 验证：2026-09-12 在 Node 24.19 下完成生产构建、271/271 测试与隐私扫描；Chromium/Firefox/WebKit 覆盖 WP1–WP5 的完整业务流程各 1/1，桌面 1440px 与移动 390px 页面走查通过。生产依赖审计 0 漏洞、Windows SQLite 10 万对象/100 万关系全规模门以及既有 PWA/可访问性证据沿用本轮开发前已通过的专项结果。构建仍有主包大于 500 kB 的非阻塞提示。
...
- 待确认/阻塞：Hermes 若要采用补丁分支、容器化或更新固定版本，需进入新的确认门；真实平台、真实数据、新生产依赖和 G8 发布决定仍未授权。P0/P1 与 G6b 文档已推送到 `codex/p0-p1-complete`，2026-09-12 GitHub Actions Windows/Linux、SQLite 与三浏览器 7/7 通过；合并 `main` 尚未执行。
...
- 代码管理：P0/P1 远端分支为 `codex/p0-p1-complete`，正式远端目标为 `https://github.com/jiezho/DDUP.git`；G6b 确认与审计提交 `417b0f4` 已推送。本轮 WP0–WP5 已提交到当前分支但尚未推送；`experiments/hermes-poc/` 继续保持未跟踪和排除。
