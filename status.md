...
- 验证：2026-09-15 Reader 并发清理修复提交 `0bc362c` 已取得 GitHub Actions 7/7 远端证据（流水线 `34986507580`）。本地前端已加入路由、搜索和阅读器按需加载及构建包体硬门：入口 258.81 kB、最大分块 464.77 kB，原 1.55 MB 主包告警已消除；274/274、隐私扫描与 Chromium/Firefox/WebKit 端到端各 1/1 通过，本轮代码待提交远端复验。
...
- 代码管理：当前远端分支为 `codex/p0-p1-complete`，正式远端目标为 `https://github.com/jiezho/DDUP.git`；WP0–WP5 基线 `c0159cb`、G7/S8 提交 `340c1b9` 与 Reader 稳定化提交 `0bc362c` 均已推送，最新已完成远端流水线为 `https://github.com/jiezho/DDUP/actions/runs/34986507580`，7/7 通过。`experiments/hermes-poc/` 继续保持未跟踪和排除。
