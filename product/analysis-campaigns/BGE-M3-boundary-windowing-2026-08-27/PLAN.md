...
| 2026-09-11 | 在本机已审查离线运行时恢复 Q1/Q2 完整评测 | 模型与 fixture 均已存在，无下载、无真实数据、无外部服务 | `window_160_40` 达到 Recall/Top-1 100% 且全部安全门通过，晋级 Q3 |
| 2026-09-11 | Q3 接入 160/40 字符窗口最大池化 | 这是完整评测最优 arm；窗口仅改变派生向量，不改变 chunk/locator 身份 | 仍保持 `experimental` 且默认关闭；健康检查锁定策略身份 |
| 2026-09-11 | 完成 100 次串行 + 10 轮双并发稳定性切片 | 验证缓存、身份和有界 busy 降级 | 100/100 串行成功；并发 10 次成功、10 次明确 busy、0 异常；不外推为生产容量或多日稳定性 |
