...
> 当前进展：G5a 已通过；Project/Task/Discussion/Decision/Capture/Today 与受控 Source/Document 已按垂直切片落地；FTS5 权限优先全文基线已验证，混合检索进入离线 POC 与评测准备
...
- 当前 `SearchProvider` 仍只有全文实现。向量、混合召回、重排和回答服务必须经过 G5a/G5b，不是现有能力。
...
- 最新完整测试 178/178 通过；Playwright Chromium 端到端 1/1 通过；
- 隐私扫描通过；
- 现有服务已验证 REST/SSE、本地路径防护、受控写入、领域持久化与权限优先全文检索；混合检索、引用回答和 Runtime 仍未实现。
