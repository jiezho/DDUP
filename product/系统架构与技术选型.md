...
- 当前 `SearchProvider` 有全文默认实现，以及 G5b 授权的受保护混合实验实现：授权范围先行、只向 token 保护的 `127.0.0.1` sidecar 发送已授权稳定 chunks、模型身份固定、证据不足或运行时/派生投影失败自动回退 FTS。chunk 身份绑定 SourceVersion、字符范围、处理版本与正文哈希；实验阈值 `0.50` 来自独立合成 calibration/blind，但因盲测间隔仅约 `0.0112`，总开关继续默认关闭。重排和回答服务仍不是现有能力。
...
- 最新完整测试 203/203 通过；Playwright Chromium 最近一次端到端 1/1 通过；
- 隐私扫描通过；
