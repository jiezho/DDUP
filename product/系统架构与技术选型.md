...
- `native-v1` 是进程内确定性生命周期执行器，只接收 ContextPackage digest 与纳入/排除计数，不调用模型或外部网络，也不产生 Answer；唯一 Runtime 可调用 Tool 是 S5-02 的 L1 Task Candidate。
...
- 失败/越权 ToolCall 只持久化参数摘要和安全错误码；正式 AI 运行中心已覆盖真实 Run、事件、Checkpoint 与 Task Candidate 审批/应用，L3/L4、其他候选类型和 Runtime 私有 resume 仍未实现。
...
- 2026-09-01 Node 24.19 完整测试与生产构建 230/230 通过，隐私扫描通过；Native Runtime 生命周期专项 7/7、Task Candidate/Approval 专项 3/3、运行中心静态契约 2/2、Harness 协议预检 4/4；运行中心桌面端/390px Playwright Chromium 1/1 通过并保存合成截图；
- 隐私扫描通过；
- 现有服务已验证 REST/SSE、本地路径防护、受控写入、领域持久化、权限优先全文检索，以及默认关闭的受保护混合检索实验切片；`native-v1` 生命周期、Task Candidate/Approval、正式运行中心、Runtime SSE、Checkpoint 与受限重试首版已实现，引用回答、重排、其他 Tool/Candidate 与 Runtime 私有 resume 仍未实现。
