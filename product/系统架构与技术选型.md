...
- DeepSeek Harness/Hermes 仍为 `connected=false` 的 POC/candidate，不参与 Run 创建或健康伪探测；Harness 已补齐 stdio JSON-RPC 握手、64 KiB 帧门、脱敏摘要、固定包供应链和官方客户端 synthetic 传输预检。G6a 因精确 npm 包与 Windows 发行物缺少 SDK 服务端/Profile 按 Stop 收口，Native 继续作为唯一可运行主 Runtime。
