...
> 当前进展：G5b 已通过；核心业务、Source 生命周期、ContextPackage/AnswerAttempt、本地保守直接引文 Answer/Claim/Citation、三类 Candidate 治理和本地备份恢复已按垂直切片落地；FTS5 是默认基线，受保护混合检索仍默认关闭；开放式改写/推理回答仍关闭
...
- migration 011/013 已建立 AnswerAttempt 安全检查、固定引用快照和最终 Answer/Claim/Citation。创建时重新授权并校验 ContextPackage 精确版本；无证据、危险问题或来源不可信指令时持久化拒答。默认 `local-extractive-v1` 只构造固定范围中的直接引文；每条声明通过保守蕴含校验才原子写入，越界或推断输出整份回滚。读取时重新复核来源状态、SourceVersion、Document、范围与 SHA-256；漂移或归档后隐藏回答和声明正文。该能力不是通用 NLI、事实核验或开放式生成。
- 提取式逐句预检复用同一授权与引用完整性边界，但不进入持久化内核：只有 `evidence_ready + integrity.verified` 才在请求内读取固定范围，将候选句限定到显式引用序号做连续原文包含检查；响应与存储都不暴露候选句或正文，不写 Audit/Outbox，不调用 Runtime。它是语义蕴含门之前的可测试安全垂直切片，不是事实验证或最终回答服务。

#### 3.6.2 S5 Native Runtime 当前落地边界（2026-09-01）

- migration 008 建立 `agent_runs/run_events`；Runtime Registry 与业务存储通过适配器边界解耦。
- `native-v1` 是进程内确定性生命周期执行器，只接收 ContextPackage digest 与纳入/排除计数，不调用外部网络；Runtime 可调用 Tool 固定为 L1 Task/Knowledge/Decision Candidate，Knowledge 引用必须完全属于本次显式范围。回答运行时是独立的本地保守提取适配器。
- 事件以持久化 `run_id + seq` 为真源，并提供有界 JSON 与可恢复 SSE 投影；migration 010 建立 Workbench Checkpoint 与 `retry_of_run_id`。进程中断遗留 Run 安全收敛为 retryable/不可重试失败，只有零 ToolCall 且范围版本仍有效的 Run 才能创建带谱系的新重试。
- DeepSeek Harness/Hermes 仍为 `connected=false` 的 POC/candidate，不参与 Run 创建或健康伪探测；Harness G6a 因 Windows 发行物缺少 SDK 服务端/Profile 按 Stop 收口。Hermes G6b 已确认，但 P1 因当前版本/依赖公告和默认完整宿主工具面在安装前 Stop。Native 继续作为唯一可运行主 Runtime。
- migration 009 建立 ToolCall、Candidate 与 Approval。Native 只允许 `candidate.task.create.v1` L1；它将用户结构化输入保存为待确认候选，不创建 Task。L2 应用经本地拥有者 Approval、scope digest、防替换、过期和版本校验后，在同一事务内创建 `ai_candidate` Task。
- 失败/越权 ToolCall 只持久化参数摘要和安全错误码；正式 AI 运行中心已覆盖真实 Run、事件、Checkpoint、三类 Candidate 预览/审批/应用/安全撤销及低敏审计。L3/L4、外部动作和 Runtime 私有 resume 仍未实现。
...
│   ├── professional/        # 科研与 AI Lab 专业对象、状态机和受控证据关系
...
当前 migration 014 在 SQLite 中增加科研问题/实验/Claim 与 AI 机会/评测专业表；这些表只补充专业字段，并以 `space_id + project_id` 复用既有权限和项目真源。专业服务通过同一 Fastify 本地 API、会话、CSRF、幂等、审计和 Outbox 边界提供，不引入第二个服务或新生产依赖。详见 ADR-023。

当前 migration 015 以同样边界增加前沿专题/固定来源信号、学习方向/练习和轻量习惯/打卡。可执行行动继续回到 Task；证据继续回到 SourceVersion/Document；按日期打卡不创建第二套 Today 状态。详见 ADR-024。
...
- Node 24.19 生产构建通过，7,416 个模块；
- `node:sqlite` Windows 可靠性 Spike 通过 9 类检查，完整规模为 10 万对象/100 万关系；
- 2026-09-02 Node 24.19 完整测试与生产构建 246/246 通过，隐私扫描通过；Citation Manifest/AnswerAttempt 安全链路专项 17/17、Native Runtime 生命周期专项 7/7、Task Candidate/Approval 专项 3/3、运行中心静态契约 2/2、Harness 协议预检 4/4；全站关键流程桌面端/390px Playwright Chromium 1/1 通过并保存合成截图；
- 2026-09-07 Node 24.19 完整测试与生产构建 250/250 通过，隐私扫描通过；Citation Manifest/AnswerAttempt/提取式预检安全链路专项 21/21。新增 Playwright 桌面/390px 用例已写入，但本轮 Chromium 启动因权限审批服务 403 未执行，不能替代 2026-09-02 的最近一次 E2E 通过证据；
- 隐私扫描通过；
- 现有服务已验证 REST/SSE、本地路径防护、受控写入、领域持久化、权限优先全文检索、AnswerAttempt 与保守 Answer/Citation、Source 版本/归档恢复、三类候选治理以及 SQLite+受控来源备份恢复；受保护混合检索仍默认关闭。开放式回答、重排、外部动作与 Runtime 私有 resume 仍未实现。
