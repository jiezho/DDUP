...
> 当前真实状态：受控 Markdown 与权限优先 FTS5 已实现；向量、混合召回、重排、引用回答均未实现
...
- 2026-08-25 验证：178/178 测试、生产构建、隐私扫描和 Playwright 1/1 通过。
...
1. 冻结 `Chunk`、`SearchCandidate`、`SearchEvidence` 与 Adapter 契约；
2. 生成独立的“模型与运行时依赖审查_待确认”清单；未确认前使用确定性假向量完成契约与权限测试；
3. 建立至少 60 条合成 qrels 和 FTS 基线报告；
4. 经依赖确认后，在隔离适配器内运行 BGE-M3 dense POC；
5. 比较 FTS、dense、RRF；只有融合达门后才评估 reranker；
6. 完成越权、错误、重复、模型不可用、索引损坏和回退测试；
7. 生成 G5b《AI 评测方案与检索评测报告_待确认》；
8. G5b Go 后再实现 Answer/Citation 服务和 UI；Stop 则保留 FTS 并关闭实验入口。
...
- [x] 用户确认 G5a-1–G5a-11；
- [x] 文件移除 `_待确认` 并记录确认日期；
- [ ] Adapter 与数据契约不绑定具体模型或向量库；
- [ ] 权限过滤发生在任何正文召回、embedding 和 rerank 之前；
- [ ] Citation 与 ObjectLocator 在契约和 UI 中明确区分；
- [ ] 固定合成评测集、FTS 对照和 Stop 路径可执行；
- [ ] 未经依赖清单确认，不下载模型、不新增生产依赖、不启用扩展；
- [ ] G5b 前不宣称语义检索、重排或引用问答已可用。
