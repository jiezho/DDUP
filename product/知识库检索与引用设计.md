...
> 当前真实状态：受控 Markdown、权限优先 FTS5 和默认关闭的受保护混合检索实验路径已实现；短集合盲测和 SourceVersion 精确 chunk 定位通过；历史 sidecar 超时已通过临时候选向量缓存与有界背压完成短样本修复，但扩展边界 Top-1 仍仅 70%，故 BGE-M3 路径维持 Stop 与默认关闭；重排与引用回答未实现
