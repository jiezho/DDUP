...
> 当前真实状态：受控 Markdown、权限优先 FTS5 和默认关闭的受保护混合检索实验路径已实现；仓库外 BGE-M3 synthetic-only 回环冒烟通过；重排与引用回答未实现
...
- 2026-08-26 最新验证：195/195 测试、独立生产构建、隐私扫描通过；最近一次 Playwright 1/1。
...
6. 完成越权、错误、重复、模型不可用和回退测试；**受保护实验切片已覆盖，索引损坏与更强盲测后续补充**；
7. 生成并确认 G5b《AI 评测方案与检索评测报告》；**2026-08-26 已完成**；
8. 继续补独立盲测、阈值校准和真实 SourceVersion 字符定位合成验证；通过后另设 Answer/Citation 启用门。
