...
> 当前真实状态：受控 Markdown 不可变版本/归档恢复/精确原文、权限优先 FTS5、显式 ContextPackage Citation Manifest、持久化 AnswerAttempt、安全拒答、无持久化提取式预检，以及本地保守直接引文 Answer/Claim/Citation 已实现。最终声明只能原样包含于所标注固定范围，写入前和读取时都复核 SourceVersion/Document/字符范围/SHA-256；来源漂移或归档后隐藏回答正文。该切片不是通用语义 NLI、事实核验或开放式生成。默认关闭的 BGE-M3 实验路径扩展边界 Top-1 仍仅 70%，继续维持 Stop；重排和开放式引用回答未实现
...
- 已有读取时派生的 Citation Manifest、可跨重启审计的 AnswerAttempt 引用快照/拒答，以及不保存候选句的逐句原文包含预检；仍没有最终 Citation/Answer 真源、答案正文和回答服务，也没有验证逐句声明与证据之间的语义蕴含关系。
...
9. 扩展长文档、噪声、混合语言和提示注入边界集；**质量未通过：回答召回 90%、Top-1 70%；sidecar 历史超时已于 2026-08-27 完成短样本修复，但不改变质量 Stop**；
10. 诊断相邻主题错排、英文长尾漏召回、重复 passage 编码和持续调用退化；只有边界、资源与背压门通过后才另设 Answer/Citation 启用门；
11. 在生成门之前先持久化 AnswerAttempt 安全检查，验证固定上下文版本、引用快照、无证据/危险意图拒答、幂等与恢复；**已完成，不生成答案**。
...
- [x] Citation 与 ObjectLocator 已在契约和 ContextPackage 正式 UI 中明确区分；无证据时显示必须拒答，生成式回答仍关闭；
- [x] AnswerAttempt 已固定 ContextPackage 版本/digest 和引用快照，并验证无证据/危险问题/来源不可信指令拒答、越权、版本、幂等与重启恢复；来源安全检查不回显命中片段，读取时复核精确来源、范围和哈希，漂移即失败关闭；生成门明确区分拒答、完整性失败与回答运行时不可用；不保存答案正文；
- [x] 提取式逐句预检已限定到授权 AnswerAttempt 和显式引用序号；精确原文可定位才通过，改写与缺失引用失败，拒答/漂移失败关闭；候选句、原文和结果均不持久化，且明确不代表语义蕴含或事实正确；
...
- [x] sidecar D1–D3 退化诊断完成：临时向量缓存与非阻塞 busy 通过短样本，长期稳定与质量门仍保持未通过；
- [ ] 后续安全门前不宣称重排或引用问答可用。
