...
Migration 008 的当前落地范围比上述目标模型更窄：`agent_runs` 绑定 `space_id + context_package_id + context_package_version + context_digest`，记录固定 Runtime/Profile 版本、目标、`max_steps`、`max_tool_calls=0`、生命周期时间、主体、乐观锁版本和安全错误码。状态暂不含 `waiting_approval`；`project_id/context_manifest_json/parent_run_id/checkpoint_ref` 尚未入库。`goal` 保存在 Run 主记录用于用户查看，但不复制进 `run_events`。ContextPackage 正文、用途和引用摘录不写入 Runtime 事件。
...
Migration 008 当前只建立 `run_events`，以 `(run_id, seq)` 唯一并使用版本化 JSON payload；`tool_calls` 将在 S5-02 建立。事件回放已经持久化并可在服务重启后读取，SSE 投递和 Checkpoint 仍待 S5-03。
...
- 当前实现：migration 001–007 已落地基础身份/空间、项目工作流、Capture、DailyPlan/Review、Source/SourceVersion/Document/索引和 ContextPackage；所有写切片共用 AuditEvent、OutboxEvent 和 IdempotencyKey；
- 已证明：UUIDv7、空间过滤、版本冲突、同项目复合外键、一层父任务、人工讨论转换的多对象原子事务、事务回滚、幂等重放/冲突/到期复用、软删除/恢复、重启持久化和 migration checksum；
- 尚未实现：KnowledgeItem/Citation/Answer、Candidate、Approval、Runtime 等后续迁移，以及完整备份/恢复工具；
- 下一步：实现通用捕获/来源入口，并保持 Task 为唯一行动真源。
