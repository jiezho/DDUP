...
- 当前实现：migration 001–003 已落地 Principal、Space、Project、Milestone、Task、Discussion、DiscussionEntry、Decision、DecisionTaskLink、AuditEvent、OutboxEvent 和 IdempotencyKey；项目与工作流专项测试 18/18；
- 已证明：UUIDv7、空间过滤、版本冲突、同项目复合外键、一层父任务、人工讨论转换的多对象原子事务、事务回滚、幂等重放/冲突/到期复用、软删除/恢复、重启持久化和 migration checksum；
- 尚未实现：Source、Knowledge、Candidate、Approval、Runtime 等后续迁移，以及完整备份/恢复工具；
