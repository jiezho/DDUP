...
同一 Source 的不可变新版本、归档恢复、精确范围读取与最终 Answer/Claim/Citation 已由后续 migration 实现；`canonical_uri/origin/trust_level/classification`、通用解析 Job 和 block map 仍未实现。`context_search` 是可重建派生索引，通过触发器覆盖 Project、Task、Capture 与 Document；删除业务对象会同步移除检索投影。
...
2026-09-01 的 Citation Manifest 前置切片没有新增业务表：读取有效 ContextPackage 时，Document 项从当前不可变正文派生 `source_id + source_version_id + document_id + char_range + text_sha256`，非 Document 项明确标为 `object_locator`，失效项标为 `excluded` 且不返回定位。包级 `answer_readiness` 只表达 `citation_manifest_ready/refused_no_citable_evidence/package_expired/package_archived`，并固定 `generation_enabled=false`。这些都是读取投影，不是 Citation/Answer 真源，也不允许据此宣称生成式问答可用。

#### migration 011 AnswerAttempt 当前落地

`answer_attempts` 是回答前安全检查真源，不是最终 Answer。它保存 `context_package_id + context_package_version + context_digest`、用户问题及 SHA-256、`evidence_ready/refused_no_citable_evidence/refused_unsafe_intent` 状态、拒答码、证据/相关/排除计数和审计主体；`generation_enabled` 只表示当前批准的本地回答适配器是否可用。migration 013 的 `answers/answer_claims/answer_citations` 才保存验证通过的最终直接引文回答与固定定位。

`answer_attempt_citations` 只在 `evidence_ready` 时保存固定引用：Source、SourceVersion、Document、半开字符范围和片段 SHA-256；不复制 quote、Document 正文或业务对象正文。危险意图与无证据拒答不保存引用快照。创建时必须重新授权并校验 ContextPackage 精确版本，过期、归档、版本冲突和不可访问空间均不产生记录；AnswerAttempt、引用快照、AuditEvent、OutboxEvent 与 IdempotencyKey 同事务提交。

`integrity` 是读取投影而非新增持久化字段：列表、单条读取和幂等重放均按固定 Source/SourceVersion/Document、字符范围与 SHA-256 复核，返回包级 `verified/invalid/not_applicable` 和引用级失败原因。来源不可用、范围失效或哈希不一致时，历史记录保持不变，但任何后续生成必须失败关闭。

`safety` 同样是读取投影。创建时先检查问题，再检查用户明确选入的 Document 字符范围；来源命中指令覆盖、角色冒充、敏感信息索取或外部外泄模式时，持久化 `refused_unsafe_intent + UNSAFE_EVIDENCE_*`，不写 `answer_attempt_citations`。状态代表确定性安全边界拒答，`safety.scope=question/evidence` 用于区分触发面；响应和审计只保存原因码，不保存命中片段。

`generation_gate` 是第三个非持久化投影：拒答记录为 `blocked_refusal`，证据完整性未验证为 `blocked_integrity`，两者均通过但没有获准回答运行时为 `blocked_model_unavailable`。当前 `available=false`、`runtime_id/profile_id=null`，不能由 Native Runtime 或候选 Runtime 状态推导为可生成。

该切片不建立 `answers` 或最终 `citations` 表，不调用模型/Runtime，也不证明问题中的逐句声明受证据蕴含。后续 Answer 不能把 `evidence_ready` 原地改写为回答；应创建独立不可变 Answer/Citation 真源并保留与 AnswerAttempt 的溯源关系。

`validate-draft` 是基于 AnswerAttempt 的无状态派生校验，不新增表或字段。请求中的候选句只在当前请求内以 NFC、统一换行和首尾去空白后，与用户标注引用序号对应的固定范围做连续包含检查；响应只含声明序号、布尔结果、原因码和命中引用序号，不含候选句或来源正文。拒答和引用完整性失败优先阻断。该投影固定 `semantic_entailment=false`、`persistence_enabled=false`、`generation_enabled=false`，不得作为 Answer、Citation、KnowledgeItem 或已验证事实的数据源。
...
Migration 009/012 已建立 `tool_calls/candidates/approvals/knowledge_items`。ToolCall 只保存参数 digest、状态、稳定错误码及 Candidate/Approval 引用；Task/Knowledge/Decision proposal 只在 Candidate 中保存，批准范围用类型、项目和 proposal SHA-256 digest 绑定。Approval 仅支持 L2 且同一 Candidate 一个审批；应用后记录目标 ID/版本，只有目标未变化且无受保护后续关系时才允许软撤销。
...
migration 014 已完成科研与 AI Lab 纵向切片：`research_questions`、`research_experiments`、`research_claims`、`ai_opportunities`、`ai_experiments` 已作为明确领域表实现。migration 015 已实现 `radar_topics`、`radar_signals`、`learning_tracks`、`learning_practices`、`learning_routines`、`learning_checkins`。科研 Claim 与前沿 Signal 都固定 SourceVersion/Document/字符范围/SHA-256，并在读取时复核证据完整性；实验、评测、专题复核和学习复盘可原子创建一个回链原 Project 的 Task。在线抓取、自动聚类/周报、音频和模型学习评分仍未实现。
...
- 当前实现：migration 001–011 已落地基础身份/空间、项目工作流、Capture、DailyPlan/Review、Source/SourceVersion/Document/索引、ContextPackage、AnswerAttempt/固定证据快照、AgentRun/RunEvent、ToolCall/Candidate/Approval、RunCheckpoint 与重试谱系；所有写切片共用 AuditEvent、OutboxEvent 和 IdempotencyKey；
- 已证明：UUIDv7、空间过滤、版本冲突、同项目复合外键、一层父任务、人工讨论转换的多对象原子事务、事务回滚、幂等重放/冲突/到期复用、软删除/恢复、重启持久化和 migration checksum；
- migration 013 已实现最终 Answer/Claim/Citation 真源；当前只接受固定原文的 `direct_quote` Claim，并在写入前执行保守蕴含校验。开放式推理/改写、通用 NLI 和 Runtime 私有 resume/steer 尚未实现；
- 备份恢复已实现本地 manifest、文件 SHA-256、对象计数、SQLite 在线快照与空目录离线恢复；跨设备分发恢复仍待发布演练；
