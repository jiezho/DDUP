...
> 前置决定：G5a-D 已确认  
> 确认记录：2026-08-25，G5a-DL1–G5a-DL8 全部采用推荐项  
> 当前真实状态：**授权范围内下载、校验、隔离安装与合成 POC 已完成；未接入 Workbench、未生产启用**
...
用户已于 2026-08-25 回复 `G5a-DL全部按推荐项确认。`。本确认授权在本文精确预算与排除项内下载、校验并建立仓库外 CPU 隔离 POC；不授权 reranker、真实资料、外部服务、生成式回答或生产启用。

### 执行结果

- 70 个 wheels 与 12 个模型文件完成下载和本地 SHA-256 重算，合计 2,558,106,823 bytes；
- 仓库外 Python 3.12 venv 完成 hash-pinned 离线安装；
- CPU-only 冒烟通过，1024 维向量，观测峰值工作集约 1.89 GiB；
- 60 条合成 dense/RRF 评测完成，质量硬门通过，但原始对抗泄漏与无答案误召回暴露风险；
- 详细结果见 `BGE-M3隔离POC运行报告.md`；G5b 已于 2026-08-26 条件式 Go，当前仅完成默认关闭的受保护混合检索实验切片，生成式回答仍关闭。
...
以上 1–6 已于 2026-08-25 执行并保留证据；第 7 项未触发。批处理包装兼容限制已如实记录，没有自动更换版本。
