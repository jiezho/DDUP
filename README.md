...
> 当前阶段：G1–G5b 已确认。核心项目闭环、受控 Markdown、权限优先全文检索，以及 feature flag 下的受保护混合检索与 FTS 自动回退已经实现；短集合盲测和 SourceVersion 精确 chunk 定位已通过，但扩展边界 Top-1 仅 70%，sidecar 10 次短样本有 6 次超时。BGE-M3 仍是仓库外、synthetic-only、CPU 实验 sidecar，并因实测 Stop 保持默认关闭。重排、引用问答、DeepSeek Harness 与 Hermes 尚未接入，不应视为现有生产能力。
...
| 混合与向量检索 | 权限前置、确定性意图拦截、稳定 chunk/字符定位、校准阈值、RRF、sidecar 身份校验与 FTS 回退 | 短集合盲测通过，但边界与短时稳定性门失败；默认关闭，BGE-M3 不进入生产依赖 |
...
当前验证快照：Node 24.19，完整测试 207/207、独立生产构建和隐私扫描通过；最近一次 UI E2E 为 Playwright Chromium 1/1。受保护混合检索另有仓库外 BGE-M3 回环冒烟、独立合成盲测、边界失败和短时稳定性失败证据。构建存在主包大于 500 kB 的非阻塞提示，后续需要路由拆包和字体裁剪。
...
2. 已完成的长文/噪声边界与短时稳定性门均失败；下一步诊断错排、英文长尾漏召回、重复 passage 编码和持续调用退化，不下载 reranker；
